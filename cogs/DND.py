...
import numpy.random
from cogs.dndhelp import helpfighter
...
class DND(commands.Cog):
    ...
    @commands.command(description="Create character with unique name")
    ...
    @commands.command(description="Set character save stats. Order:armor, str, dex, con, int, wis, cha")
    async def dnd_setsaves(self, ctx, char, *dum):
        l = list(map(int, list(dum)))
        self.chars[char]["saves"] = l
        await ctx.send(self.chars)

    @commands.command(description="Set character abilityscore stats. Order:str, dex, con, int, wis, cha")
    async def dnd_setas(self, ctx, char, *dum):
        l = list(map(int, list(dum)))
        self.chars[char]["abilityscore"] = l
        await ctx.send(self.chars)

    @commands.command(description="set character skill stats. Order: Dnd sheet top to bottom")
    ...
    #add weapon
    @commands.command(description="Give character a weapon. Usage: char weaponname, attackbonus, used dice, damagetypebonus")
    ...
    @commands.command(description="Equip weapon for character.")
    ...
    @commands.command(description="Set character max hp, also heal to full.")
    async def dnd_setmaxhp(self, ctx, char, hp):
        self.chars[char]["maxhp"] = int(hp)
        self.chars[char]["hp"] = int(hp)

    @commands.command(description="Add/remove health from character.")
    ...
    @commands.command(description="Add/remove party money.")
    async def dnd_money(self, ctx, amount):
        self.money += int(amount)
        await ctx.send(f"Balance:{self.money}")

    @commands.command(description="Show how much money the party has.")
    async def dnd_balance(self, ctx):
        await ctx.send(f"Balance:{self.money}")

    @commands.command(description="Do a skill check. Order: char, skilltype, skill")
    ...
    @commands.command(description="Roll if character hit landed.")
    async def dnd_hitroll(self, ctx, char):
        weapon = self.chars[char]["weapon"]
        dice = numpy.random.randint(1,21)
        await ctx.send(f"{dice} +{weapon[0]} ={dice +weapon[0]}")

    @commands.command(description="Roll damage done by character.")
    ...
    @commands.command(description="Roll given dice and add optional bonus. Example: 1d20 3")
    ...
    @commands.command()
    async def dnd_helptest(self, ctx):
        await ctx.send(embed=helpfighter(ctx))
